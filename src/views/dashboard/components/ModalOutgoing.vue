<template>
  <a-modal v-bind:open="props.visible" :footer="null" @cancel="emits('close')" centered>
    ...
    >
      ...
      <a-row :gutter="16">
        <a-col :span="6">
          <a-form-item label="Stock" name="stock" :rules="[{ required: true }]">
            <a-input-number
              :min="0"
              placeholder="Stock"
        ...
        <a-col :span="6">
          <a-form-item label="Used" name="quantity_out" :rules="[{ required: true }]">
            <a-input-number
              :min="0"
              placeholder="Used"
...
watch(
  ...
  (newData) => {
    if (newData) {
      ...
      form.value.stock = newData.actual_stock || 0
    } else {
      handleReset()
    }
  },
...
watch(
  () => form.value.quantity_out,
  (newQuantity) => {
    if (newQuantity > form.value.stock) {
      errorMessage.value = `Used can't be greater than stock`
    } else {
      errorMessage.value = ''
    }
  }
